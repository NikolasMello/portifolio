...
export default function Header() {
  ...
  return (
    <header className='w-full fixed backdrop-blur-xl z-[9]'>
      <nav
        className={
          navBar
            ? `mx-auto flex max-w-7xl items-center justify-between p-2 mt-2 rounded-3xl navbar-shadow lg:px-8 transition-all duration-500`
        ...
        <Popover.Group className='hidden lg:flex lg:gap-x-12 hover:transition-all'>
          {menuItens.map(({ name, link }) => (
            <Link
              ...
              href={link}
              className={
                verifyActiveItem(link) ? 'text-link-active' : 'text-link'
              }
            >
              {name}
            </Link>
            ...
            <div className='-my-6 divide-y divide-slate-600/10 dark:divide-slate-400/10'>
              <div className='space-y-2 py-6'>
                {menuItens.map(({ name, link }) => (
                  <Link
                    ...
                    onClick={() => setMobileMenuOpen(false)}
                    className={
                      verifyActiveItem(link)
                        ? '-mx-3 block rounded-3xl px-3 py-2 text-base text-link-active overflow-hidden'
                        : '-mx-3 block rounded-3xl px-3 py-2 text-base text-link overflow-hidden'
                    }
                  >
                    {name}
                  </Link>
              ...
              <div className='py-6'>
                <a
                  ...
                  target='_blank'
                  className='-mx-3 block rounded-3xl px-3 py-2.5 text-base font-semibold leading-7 text-slate-600 hover:text-purple-700 overflow-hidden'
                >
                ...
                <a
                  href='https://www.linkedin.com/in/nikolas-mello-a623481b2/'
                  target='_blank'
                  className='-mx-3 block rounded-3xl px-3 py-2.5 text-base font-semibold leading-7 text-slate-600 hover:text-indigo-700 overflow-hidden'
                >
                  <div className='flex items-center'>
